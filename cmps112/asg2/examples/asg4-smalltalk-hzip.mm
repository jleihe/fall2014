...
.SIZE 12 14
.INITR* \n[.F]
.TITLE CMPS-112 Fall\~2014 Project\~2 \
"Object Oriented Smalltalk"
.RCS "$Id: asg4-smalltalk-hzip.mm,v 1.10 2014-10-22 15:47:12-07 - - $"
.PWD
.URL
.de ULCB
...
the program is
.V= gst ,
and you should make sure you have
.VTCODE* 1 "export \
PATH=\[Do]PATH:/afs/cats.ucsc.edu/courses/cmps112-wm/usr/smalltalk/bin"
or equivalent.
For your own computer, the sources may be obtained from\(::
...
.H 1 "Biography"
Professor David Huffman is best known for his Huffman code,
an optimal compression scheme for lossless variable length encoding.
In a fixed length encoding scheme, all characters have the same length.
For instance, each ISO-Latin-1 character consumes 8 bits.
On the other hand,
the Huffman code uses a variable length encoding in which the most
frequently used character is represented by the fewest bits.
A message is thereby described in the most compact form.
In fact,
no other scheme can produce a shorter representation
of a message in the general case.
.P
The genesis of this algorithm is as fascinating as the algorithm
itself.
In 1950,
Huffman enrolled in a graduate class
in information theory and coding at MIT.
The instructor, Professor Robert Fano,
gave students the option of taking a final exam or solving one
of several interesting problems.
Huffman opted for the latter because he ``decided it might be
easier to solve a seemingly easy problem than it would be to
face a final exam''.
About a week before the final exam,
Huffman threw a stack of unsuccessful,
attempted solutions into the trash.
This action somehow caused Huffman to realize the solution and
write the term paper.
.P
After completing his graduate studies,
Huffman joined the faculty at MIT.
In 1967, he left MIT to become the founding faculty member
of the Computer Science Dept.\& at the University of California,
Santa Cruz.
.=V [ http://www.ucsc.edu/currents/99-00/10-11/huffman.html \(;;
.V= http://www.acm.org/crossroads/xrds6-3/huffman.html ]
.H 1 "The Jargon File being Disrespectful"
.ULCB huff \(::
vt. 
...
.ULCB crunch ,
.ULCB compress .
.=V [ http://www.catb.org/\[ti]esr/jargon/html/H/huff.html ]
.P
.ULCB puff \(::
vt. 
...
see
.ULCB inflate .
.=V [ http://www.catb.org/\[ti]esr/jargon/html/H/puff.html ]
.FINISH
